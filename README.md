...
## Этап 4. Создание заявки на аукцион

### What was implemented

- Добавлена backend-модель аукциона со статусом `pending` для проверки модератором.
- Верифицированные пользователи могут отправлять заявку на создание аукциона через `POST /api/auctions`.
- Пользователь может смотреть свои заявки через `GET /api/auctions/my`.
- Добавлена загрузка до 50 фотографий предмета торгов с выбором главной фотографии.
- Добавлена серверная валидация:
  - создавать аукционы могут только обычные пользователи со статусом `approved`;
...
- В личный кабинет добавлены вкладки `Верификация`, `Мои аукционы`, `Создать аукцион`.
- Форма создания аукциона вынесена в отдельные frontend-компоненты и не расширяет `App.jsx`.
...
3. Open the cabinet and go to `Создать аукцион`.
4. Fill price, dates, item info, characteristics, contacts, upload at least one photo and choose the main photo.
5. Click `Подать заявку на создание аукциона`.
6. Open `Мои аукционы` and check that the lot appears with status `Ожидает проверки`.
